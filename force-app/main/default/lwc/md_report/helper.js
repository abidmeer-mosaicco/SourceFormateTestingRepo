...
const isDateFieldName = (name) => {
  ...
  return n.includes("date") || n.endsWith("start__c") || n.endsWith("end__c");
};
...
export function buildManualColumns(manualFields = [], manualLabels = []) {
  ...
  return (manualFields || []).map((apiName, idx) => {
    ...
    if (isDateFieldName(apiName)) {
      return { label, fieldName: apiName, type: "text", editable: false, initialWidth: 180, isDateColumn: true };
    }

    let tmpColumn = { label, fieldName: apiName, type: "text", editable: false, wrapText: true, initialWidth: 180 };
    return tmpColumn;
  });
}
