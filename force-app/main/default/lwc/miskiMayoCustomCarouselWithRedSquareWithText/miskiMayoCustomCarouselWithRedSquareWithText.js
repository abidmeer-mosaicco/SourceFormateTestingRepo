...
export default class MiskiMayoCustomCarouselWithRedSquareWithText extends LightningElement {
    slides = [];
    slideIndex = 1;
    timer;
    textTimeout;
    ...
    // === NEW PROPERTIES FOR THE LEFT COLUMN ===
    @api paragraph1;  @api paragraph2;  @api paragraph3;  @api paragraph4;  @api paragraph5;
    @api paragraph6;  @api paragraph7;  @api paragraph8;  @api paragraph9;  @api paragraph10;
    @api fontSizeParagraphs; // Ex.: "16px", "1rem"

    @api titleparagraph1; @api titleparagraph2; @api titleparagraph3; @api titleparagraph4; @api titleparagraph5;
    @api titleparagraph6; @api titleparagraph7; @api titleparagraph8; @api titleparagraph9; @api titleparagraph10;
    @api checkMarkColor;
    ...
    get checkMarkStyle() {
        return `color: ${this.checkMarkColor}; font-weight: bold; margin-right: 10px;`;
    }

    get paragraphItems() {
        const ps = [
            { t: this.titleparagraph1, v: this.paragraph1 },
            { t: this.titleparagraph2, v: this.paragraph2 },
            { t: this.titleparagraph3, v: this.paragraph3 },
            { t: this.titleparagraph4, v: this.paragraph4 },
            { t: this.titleparagraph5, v: this.paragraph5 },
            { t: this.titleparagraph6, v: this.paragraph6 },
            { t: this.titleparagraph7, v: this.paragraph7 },
            { t: this.titleparagraph8, v: this.paragraph8 },
            { t: this.titleparagraph9, v: this.paragraph9 },
            { t: this.titleparagraph10, v: this.paragraph10 }
        ];

        return ps
            .map((row, i) => ({
                key: `p${i+1}`,
                title: (row.t || '').toString().trim(),
                value: (row.v || '').toString().trim()
            }))
            .filter(p => p.title || p.value);
    }

    get hasParagraphs() { return this.paragraphItems.length > 0; }

    get paragraphFontSizeStyle() {
        const v = (this.fontSizeParagraphs || '').toString().trim();
