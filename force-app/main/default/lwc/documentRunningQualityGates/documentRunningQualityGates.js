...
export default class DocumentRunningQualityGates extends LightningElement {
    @api sectionValue = 'Running Quality Gates';  // exact picklist label
    @api pageTitle = 'Running Quality Gates';
    ...
    handleSearch = (e) => { const v = (e.target.value || '').trim(); this.searchTerm = v; clearTimeout(this._debounce); this._debounce = setTimeout(() => this.applySearch(), 180); };
    applySearch() {
        const q = (this.searchTerm || '').toLowerCase();
        if (!q) { this.noResults = false; this.topics = JSON.parse(JSON.stringify(this.allTopics)); requestAnimationFrame(() => this.renderBodies()); return; }
        const filtered = this.allTopics.map(t => {
            const arts = (t.articles || []).filter(a =>
                (a.title && a.title.toLowerCase().includes(q)) || (a._searchBlob && a._searchBlob.includes(q))
            );
            return arts.length ? { ...t, _expanded: true, articles: arts } : null;
        }).filter(Boolean);
        this.noResults = filtered.length === 0; this.topics = filtered; requestAnimationFrame(() => this.renderBodies());
    }
    renderBodies() {
