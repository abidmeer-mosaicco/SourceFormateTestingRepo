...
const DEBUG = (...args) => {
  if (hasDebug) console.log("[Md_reportDownloadXls]", ...args);
};

const LOADING_NAME = 'md_report_xls_export';

export default class Md_reportDownloadXls extends LightningElement {
  @api objectApiName;
  @api rows = [];
  @api allRows = [];
  ...
  async stopGlobalLoader() {
    try {
      this.dispatchEvent(new CustomEvent('loadingEvent', { bubbles: true, composed: true, detail: { action: 'stop', name: LOADING_NAME } }));
    } catch (e) {
      // ignore
    }
    // ensure internal state is consistent
    try { this.isLoading = false; } catch (e) { /* ignore */ }
  }
  ...
  isTextColumn(fieldName, label) {
    ...
    if (normalizedFieldName.includes("sapordernumber")) return false;
    if (normalizedLabel === "order" || normalizedLabel === "order number") return true;
  ...
  async handleDownload() {
    ...
    try {
      this._toggleGlobalLoading('start');
      // this.showLoadingOverlay();

      if (this._needsParentData()) {
        DEBUG('[MdReportDownloadXls] Solicitando dados completos do parent...');
        this._waitingForParent = true;
        this._startParentWaitingTimer();
        this.dispatchEvent(new CustomEvent('requestdownload', { bubbles: true, composed: true }));
        return;
      }

      DEBUG('[MdReportDownloadXls] Iniciando download com', (this.allRows?.length || this.rows?.length || 0), 'registros...');

      this._clearParentWaitingTimer();

      const safeRows = this._getSafeRows();
      let columns = this.resolveExportColumns();

      if (!safeRows.length || !columns.length) {
        await this.stopGlobalLoader();
        this._showToast("Warning", "No records to export.", "warning");
        return;
      }

      this._prepareRowsForExport(safeRows, columns);
      columns = this._filterExportColumns(columns);

      await this._executeDownload(columns, safeRows);
    } catch (err) {
      ...
      if (!this._waitingForParent) {
        await this.stopGlobalLoader();
      }
    }
  }

  _startParentWaitingTimer() {
    this._clearParentWaitingTimer();
    DEBUG('[MdReportDownloadXls] Aguardando dados do parent...');
    this._parentWaitingTimer = setTimeout(() => {
      if (this._waitingForParent) {
        DEBUG('[MdReportDownloadXls] Timeout aguardando dados do parent (10 minutos).');
        this._waitingForParent = false;
        this.stopGlobalLoader();
        this._showToast('Erro', 'Tempo limite excedido ao buscar dados. Por favor, tente novamente com menos filtros.', 'error');
      }
    }, 600000);
  }

  _clearParentWaitingTimer() {
    if (this._parentWaitingTimer) {
      clearTimeout(this._parentWaitingTimer);
      this._parentWaitingTimer = null;
    }
  }
  ...
  _getSafeRows() {
    return Array.isArray(this.allRows) && this.allRows.length ? this.allRows : (Array.isArray(this.rows) ? this.rows : []);
  }

  _prepareRowsForExport(rows, columns) {
    const orderCol = columns.find(c => {
      const n = String(c.fieldName || '').toLowerCase();
      const l = String(c.label || '').toLowerCase();
      if (n.includes('sapordernumber')) return false;
      return l === 'order number' || n.endsWith('.ordernumber');
    });

    for (const row of rows) {
      try {
        row['export_order_number'] = this.getExportOrderValue(row, columns);
        if (orderCol) row[orderCol.fieldName] = row['export_order_number'];
      } catch (e) { /* ignore */ }
    }
  }

  _filterExportColumns(columns) {
    return (columns || []).filter(col => {
      ...
      if ((lname === 'order number' || fname.endsWith('.ordernumber')) && !fname.includes('sapordernumber')) {
        return { ...col, fieldName: 'export_order_number' };
      }
      return col;
    });
  }

  async _executeDownload(columns, rows) {
    const blob = this.buildXlsxBlob(columns, rows);
    ...
    const ts = `${MM}_${DD}_${YYYY}_${HH}_${MIN}`;
    const namePrefix = this.exportFileName || this.objectApiName || "export";

    a.href = url;
    a.download = `${namePrefix}_${ts}.xlsx`;
    document.body.appendChild(a);
    a.click();
    a.remove();

    await new Promise(r => setTimeout(r, 1500));
    this._setupRevokeTimer(url);
  }

  _setupRevokeTimer(url) {
    const revoke = () => {
  ...
  _toggleGlobalLoading(action) {
    this.dispatchEvent(new CustomEvent('loadingEvent', {
      bubbles: true,
      composed: true,
      detail: { action, name: LOADING_NAME }
    }));
  }
  ...
  @api
  async downloadTable(rows) {
    DEBUG('[MdReportDownloadXls] downloadTable chamado com', rows?.length || 0, 'registros');
    this._clearParentWaitingTimer();
    this._waitingForParent = false;

    if (Array.isArray(rows) && rows.length) {
      this.allRows = rows;
      DEBUG('[MdReportDownloadXls] allRows atualizado:', this.allRows.length, 'registros');
    } else {
      DEBUG('[MdReportDownloadXls] downloadTable chamado sem dados válidos');
    }
